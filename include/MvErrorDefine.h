...
#define MV_E_PRECONDITION           0x80000008  ///< \~chinese ???????????????§Ý?????????£   \~english Precondition error, or running environment changed
#define MV_E_VERSION                0x80000009  ///< \~chinese ?·Ú?????               \~english Version mismatches
...
#define MV_E_NOOUTBUF               0x8000000D  ///< \~chinese ??§á?????????         \~english No Avaliable Buffer
...
#define MV_E_UNKNOW                 0x800000FF  ///< \~chinese ¦Ä??????               \~english Unknown error
/// @}

/********************************************************************/
///  \~chinese
///  \name GenICam??§Õ???:??¦¶0x80000100-0x800001FF
///  @{
///  \~english
...
#define MV_E_GC_RUNTIME             0x80000104  ///< \~chinese ???§Ý?????????           \~english Running environment error
...
#define MV_E_GC_UNKNOW              0x800001FF  ///< \~chinese GenICam¦Ä?????          \~english GenICam unknown error
/// @}
...
///  @{
#define MV_E_NOT_IMPLEMENTED        0x80000200  ///< \~chinese ???????õô???         \~english The command is not supported by device
#define MV_E_INVALID_ADDRESS        0x80000201  ///< \~chinese ????????????????     \~english The target address being accessed does not exist
#define MV_E_WRITE_PROTECT          0x80000202  ///< \~chinese ?????????§Õ           \~english The target address is not writable
#define MV_E_ACCESS_DENIED          0x80000203  ///< \~chinese ?õô????????           \~english No permission
#define MV_E_BUSY                   0x80000204  ///< \~chinese ?õô???????????       \~english Device is busy, or network disconnected
...
#define MV_E_KEY_VERIFICATION       0x8000020F  ///< \~chinese ???§µ?????              \~english SwitchKey error
#define MV_E_IP_CONFLICT            0x80000221  ///< \~chinese ?õôIP???               \~english Device IP conflict
/// @}
...
#define MV_E_USB_WRITE              0x80000301  ///< \~chinese §Õusb????                \~english Writing USB error
#define MV_E_USB_DEVICE             0x80000302  ///< \~chinese ?õô??                 \~english Device exception
...
#define MV_E_USB_DRIVER             0x80000305  ///< \~chinese ????????????¦Ä?????   \~english Driver mismatch or unmounted drive
#define MV_E_USB_UNKNOW             0x800003FF  ///< \~chinese USB¦Ä??????            \~english USB unknown error
/// @}
...
#define MV_E_UPG_CONFLICT           0x80000402  ///< \~chinese ??????????õô???????????????????????????????? \~english Upgrading conflicted (repeated upgrading requests during device upgrade)
#define MV_E_UPG_INNER_ERR          0x80000403  ///< \~chinese ??????õô??????????   \~english Camera internal error during upgrade
#define MV_E_UPG_UNKNOW             0x800004FF  ///< \~chinese ?????¦Ä?????           \~english Unknown error during upgrade
/// @}
